...
describe("computeFxSnapshot", () => {
  ...
  it("uses fallback rate when fetchUsdNprRate returns fallback (never returns null)", async () => {
    mockFetchRate.mockResolvedValueOnce({ rate: 133.0, lastUpdated: "2024-01-01T00:00:00Z" });
    const result = await computeFxSnapshot(dec(100), "USD");
    expect(result).not.toBeNull();
    expect(result.amountUsd.toString()).toBe("100");
    expect(result.amountNpr.toNumber()).toBeCloseTo(13300, 0);
  });
  ...
  it("never returns null even when API fails (uses fallback)", async () => {
    mockFetchRate.mockResolvedValueOnce({ rate: 133.0, lastUpdated: "2024-01-01T00:00:00Z" });

    const result = await computeFxSnapshot(dec(100), "USD");
    expect(result).toBeDefined();
    expect(result.amountUsd).toBeDefined();
    expect(result.amountNpr).toBeDefined();
  });
});
